...
update_ip_in_file() {
    ...
    if [ -n "$search_pattern" ]; then
        ...
        echo -e "${GREEN}Updated IP in $config_file.${TEXTRESET}"
    else
...
check_and_update_kibana_ip() {
    ...
    if [ -n "$server_host_ip" ] && [ "$interface_ip" != "$server_host_ip" ]; then
        ...
        echo -e "${GREEN}Updated server.host IP in $config_file.${TEXTRESET}"
        changed=1
    fi
    if [ -n "$elasticsearch_hosts_ip" ] && [ "$interface_ip" != "$elasticsearch_hosts_ip" ]; then
        ...
        echo -e "${GREEN}Updated elasticsearch.hosts IP in $config_file.${TEXTRESET}"
        changed=1
    fi
